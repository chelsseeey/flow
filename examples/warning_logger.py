    ...
    def __call__(self, env, worker):
        try:
            if not self.trial_name:
                self.trial_name = str(worker.trial_name) if hasattr(worker, 'trial_name') else None
            ...
            # Store only basic types
            self.iteration_collisions.append(int(self.collision_count))
