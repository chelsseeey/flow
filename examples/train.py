...
# Flow 환경 파라미터 (설정만; 실제 환경 생성 및 등록은 main() 내부에서 수행)
flow_params = dict(
    exp_tag=exp_tag,
    # env_name과 network는 우선 문자열로 정의합니다.
    ...
    net=NetParams(additional_params={}),  # 이후 main()에서 실제 네트워크 인스턴스와 추가 파라미터로 업데이트
    veh=VehicleParams(),
...
# 차량 및 신호등 설정 (단, 여기서는 설정만 정의)
flow_params["veh"].add(
    veh_id="rl",
    acceleration_controller="RLController",  # 나중에 main()에서 실제 클래스로 업데이트 가능
...
# ---------------------------
def main():
    print("[DEBUG] Entering main() in exp1.py.")
    # Delay Flow module imports until here to prevent module-level side-effects.
    ...
    # 실제 환경 클래스 및 네트워크 인스턴스로 업데이트
    flow_params["env_name"] = MultiAgentAccelPOEnv
    flow_params["network"] = FigureEightNetwork
    # 추가 파라미터에 "radius_ring"와 같이 필요한 파라미터가 있다면 추가합니다.
    additional_net_params = deepcopy(ADDITIONAL_NET_PARAMS)
    if "radius_ring" not in additional_net_params:
        additional_net_params["radius_ring"] = 100  # 예: 100으로 설정
    flow_params["net"] = NetParams(additional_params=additional_net_params)

    # 실제 컨트롤러 업데이트 (필요하다면 실제 클래스로 변경)
    # 예를 들어:
    # from flow.controllers import RLController as ActualRLController
